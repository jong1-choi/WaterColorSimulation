...
#include "JGL/JGL__Draw.hpp"
#include "JGL/JGL__Scrollable.hpp"
...
struct ImageViewerBase: public Widget, public _Targettable {
	...
	ImageViewerBase(float xx, float yy, float ww, float hh, const string& title="" );
	
	virtual idim2 imageSize() const = 0;
	...
	void fitToScreen();
	template<typename T> glm::vec<2,T> screenToImage( const glm::vec<2,T>& pt ) const;
	template<typename T> glm::vec<2,T> imageToScreen( const glm::vec<2,T>& pt ) const;
	...
	template<typename T> glm::vec<2,T> screenToImage( const glm::vec<2,T>& pt, const glm::vec2& offset ) const;
	template<typename T> glm::vec<2,T> imageToScreen( const glm::vec<2,T>& pt, const glm::vec2& offset ) const;
	...
	virtual _Scrollable& scroller() { return _scroller; }
	virtual const _Scrollable& scroller() const { return _scroller; }
	virtual float		viewScale() const { return _viewScale; }
	virtual float		desiredScale() const { return _desiredScale; }
	virtual bool		fitted() const { return _fitted; }
	virtual bool		keepFitted() const { return _keepFitted; }

protected:

	virtual void	drawImage( const glm::mat3& mat=glm::mat3(1) ) = 0;
	virtual void	drawImage( const glm::vec2& v, float s ) = 0;
...
template<typename T> inline glm::vec<2,T> ImageViewerBase::screenToImage( const glm::vec<2,T>& pt, const glm::vec2& offset ) const {
	glm::vec<2,T> pti;
	pti.x = T((pt.x+offset.x)/_viewScale);
	pti.y = T((pt.y+offset.y)/_viewScale);
	return pti;
}
template<typename T> inline glm::vec<2,T> ImageViewerBase::screenToImage( const glm::vec<2,T>& pt ) const {
	glm::vec<2,T> pti;
...
template<typename T> inline glm::vec<2,T> ImageViewerBase::imageToScreen( const glm::vec<2,T>& pt, const glm::vec2& offset ) const {
	glm::vec<2,T> pti;
...
template<typename T> inline glm::vec<2,T> ImageViewerBase::imageToScreen( const glm::vec<2,T>& pt ) const {
	glm::vec<2,T> pti;
...
struct TexImage {
	...
	void* _data = nullptr;				//!< The copy of the pixel data, which is GL_BYTE (unsigned). Note this will be immediately deleted when the image is sent to OpenGL.
	bool _BGR = true;					//!< The flag indicating If it is BGR image. For OpenCV compatibility. \sa isBGR()
	bool _float = false;				//!< The flag indicating if the data is in float.
	bool _sRGB = true;					//!< The flag indicating if the data is in sRGB format
	...
	//! \Brief Set the user-sent image data.
	//! \Param d The pointer to the user-sent pixel data. This must be float (32bit). Note that the data will be copyed, so you must delete the original data by your self.
	//!        This data will be rendered through sRGB encoding
	//! \Param ww The width of the user-sent image.
	//! \Param hh The height of the user-sent image.
	//! \Param channels The number of channels of the user-sent image. This must be 3 or 4.
	//! \Param isBGR The flag indicating if the pixel data is in BGR form instead of RGB. The defailt value is true.
	virtual void setImage( const float* d, int ww, int hh, int channels, bool sRGB, bool isBGR=true );
...
struct ImageViewer: public ImageViewerBase {
	...
	virtual idim2 imageSize() const override { return _texImage.imgSize(); };
...
protected:
	...
	virtual void	drawImage( const glm::mat3& mat=glm::mat3(1) ) override;
	virtual void	drawImage( const glm::vec2& offset, float scale ) override;
