...
inline float tonemap(float u, float gamma) {
	if( abs(gamma-2.4f)<0.0001f ) {
		float u_ = abs(u);
		return u_>0.04045f?(glm::sign(u)*powf((u_+0.055f)/1.055f,2.4f)):(u/12.92f);
	}
	else
		return glm::sign(u)*pow(abs(u),gamma);
}
inline float invTonemap(float u, float gamma) {
	if( abs(gamma-2.4f)<0.0001f ) {
		float u_ = abs(u);
		return  u_>0.0031308f?( glm::sign(u)*1.055f*powf( u_,0.41667f)-0.055f):(12.92f*u);
	}
	else
		return glm::sign(u)*pow(abs(u),1/gamma);
}

inline glm::vec3 tonemap(const glm::vec3& rgb, float gamma) {
	return glm::vec3(tonemap(rgb.r, gamma),tonemap(rgb.g, gamma),tonemap(rgb.b, gamma));
}
inline glm::vec3 tonemap(const glm::vec3& rgb, const glm::vec3& gamma) {
	return glm::vec3(tonemap(rgb.r, gamma.r),tonemap(rgb.g, gamma.g),tonemap(rgb.b, gamma.b));
}
inline glm::vec3 invTonemap(const glm::vec3& rgb, float gamma) {
	return glm::vec3(invTonemap(rgb.r, gamma),invTonemap(rgb.g, gamma),invTonemap(rgb.b, gamma));
}
inline glm::vec3 invTonemap(const glm::vec3& rgb, const glm::vec3& gamma) {
	return glm::vec3(invTonemap(rgb.r, gamma.r),invTonemap(rgb.g, gamma.g),invTonemap(rgb.b, gamma.b));
}
extern glm::vec3 windowColor( JGL::Widget* widget, const glm::vec3& color );
extern glm::vec4 windowColor( JGL::Widget* widget, const NVGcolor& color );
