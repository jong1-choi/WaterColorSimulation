...
typedef void (*DNDCallback)(Widget*,void* ud, const std::vector<std::string>& filename );
...
// Those function is used by the enclouser and the JGL
	...
	virtual void		setDrawCB  (function<void(NVGcontext*,const glm::rect&)> f) { drawCB=f; }
	virtual void		setDrawGLCB(function<void(const glm::rect&)> f) { drawGLCB=f; }
	virtual void		unsetDrawCB() { drawCB=defDrawCB; }
	virtual void		unsetDrawGLCB() { drawGLCB=defDrawGLCB; }
...
// Action states
	...
	const std::function<void(NVGcontext*,const glm::rect&)> defDrawCB = [](NVGcontext*,const glm::rect&){};
	const std::function<void(const glm::rect&)> defDrawGLCB = [](const glm::rect&){};

	std::function<void(NVGcontext*,const glm::rect&)> drawCB = defDrawCB;
	std::function<void(const glm::rect&)> drawGLCB = defDrawGLCB;
