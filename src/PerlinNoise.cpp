...
// Standard reference permutation vector from Ken Perlin's original implementation
PerlinNoise::PerlinNoise() {
    ...
    // Duplicate for seamless wrap-around indexing
    m_permutation.insert(m_permutation.end(), m_permutation.begin(), m_permutation.end());
}

// Seeded constructor — produces a reproducible but different permutation per seed
...
double PerlinNoise::noise(double x, double y, double z) const {
    ...
    // Compute smoothstep fade curves
    double u = fade(x);
    double v = fade(y);
    double w = fade(z);

    // Hash the 8 cube-corner coordinates
    ...
    // Interpolate the gradient contributions from all 8 corners
...
// Maps the lower 4 bits of hash to one of 12 gradient directions
